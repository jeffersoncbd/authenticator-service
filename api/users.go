...
func (api API) UsersList(w http.ResponseWriter, r *http.Request, id string) *spec.Response {
	...
	if err := utils.CheckPermissions(r.Context(), usersIdentifier, utils.KeyToRead); err != nil {
	...
	if err != nil {
		return spec.UsersListJSON400Response(spec.Error{Feedback: utils.INVALID_APPLICATION_ID + err.Error()})
	}
	...
	if err != nil {
		...
		return spec.UsersListJSON500Response(spec.InternalServerError{Feedback: utils.INTERNAL_SERVER_ERROR})
	}
...
func (api API) NewUser(w http.ResponseWriter, r *http.Request, id string) *spec.Response {
	// Verifica se requisição possui a permissão necessária
	if err := utils.CheckPermissions(r.Context(), usersIdentifier, utils.KeyToWrite); err != nil {
	...
	applicationUUID, err := uuid.Parse(id)
	if err != nil {
		return spec.NewUserJSON400Response(spec.Error{Feedback: utils.INVALID_APPLICATION_ID + err.Error()})
	}
	...
	if !errors.Is(err, pgx.ErrNoRows) {
		...
		return spec.NewUserJSON500Response(spec.InternalServerError{Feedback: utils.INTERNAL_SERVER_ERROR})
	}
...
func (api API) UserUpdate(w http.ResponseWriter, r *http.Request, id string, byEmail openapi_types.Email) *spec.Response {
	...
	applicationUUID, err := uuid.Parse(id)
	if err != nil {
		return spec.UserUpdateJSON400Response(spec.Error{Feedback: utils.INVALID_APPLICATION_ID + err.Error()})
	}
	...
	if user.NewPassword != nil && *user.NewPassword != "" {
		...
		if err != nil {
			api.logger.Error("Falha ao gerar hash de senha", zap.Error(err))
			return spec.UserUpdateJSON500Response(spec.InternalServerError{Feedback: utils.INTERNAL_SERVER_ERROR})
		}
